 ...
 * Author: Mandisi M. Sibanda (20146817), Qinyuan Liu (20137095)
 */
#include <stdio.h> /* Required for sprintf */
#include <avr/io.h>
#include <avr/interrupt.h>
#define DLY_12_5_ms 61
#define COUNT_FOR_10ms 10
#define ADC_VCF 5 // ADC_Votalge_Covertion_Factor 4.8828125
...
void Init_USART(void)
{
	...
	UBRR0 = 51;											  /*baud rate = 19200, USART 2X = 0 so UBRR0 = ((16*10^6)/(16*19200))-1 = 51.08333333, rounded to 51 */
...
void chooseChar(char ch, char *buffer)
{
	...
	case 's':
	case 'S':
		sprintf(buffer, "OCR2B register: %i", OCR2B);
		sendmsg(buffer);
		break;

	case 't':
	case 'T':
		sprintf(buffer, "555 Timer period %ius", t_period);
		sendmsg(buffer);
		break;
	...
	case 'c':
	case 'C':
		sprintf(buffer, "Timer Report Continuously");
		sendmsg(buffer);
		TIMER_report_flag = 1;
		break;
	...
	case 'v':
	case 'V':
		ADC_VR = adc_reading * ADC_VCF;
		sprintf(buffer, "Report the ADC conversion result in %i mV", ADC_VR);
		sendmsg(buffer);
		break;
...
ISR(TIMER0_OVF_vect)
{
	...
	TCNT0 = DLY_12_5_ms; /*  TCNT0 needs to be set to the start point each time        */
