...
double ADC_VCf = 4.8828125;			  // ADC_Votalge_Covertion_factor
double ADC_VR;						  // ADC_Votalge_Reading
...
void chooseChar(char ch, char *buffer)
{
	...
	case 'a':
	case 'A':
		sprintf(buffer, " Report the ADC conversion result. This is the ADC value: %i.", adc_reading);
		sendmsg(buffer); /*send first message*/
		break;
	...
	case 'V':
		...
		sprintf(buffer, "Report the ADC conversion result in mV. You must convert the ADC value to mV. You must also display the ADC source.%.2f", ADC_VR);
		sendmsg(buffer); /*send first message*/
		break;
